...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function renderModules() {
    ...
    if (Array.isArray(data)) {
      data.forEach(entry => {
        ...
        // Leading emoji / icon (if provided) — shown as the bullet/lead
        const leading = document.createElement('span');
        const iconText = entry.emoji || entry.icon || entry.status || '';
        leading.textContent = iconText ? iconText + ' ' : '';
        leading.className = 'module-icon';
        ...
        // If there is a dedicated status/icon field, show it before the site link
        const statusText = entry.status_icon || entry.status || entry.emoji || '';
        if (statusText) {
        ...
        // site link (if present) - always append last
        if (entry.pages_url) {
      ...
      Object.keys(data).forEach(key => {
        ...
        const title = document.createElement('strong');
        title.textContent = (entry.name || entry.title || key) + (entry.description ? '' : '');
        ...
        // Add site link if present (always last)
        if (entry.pages_url) {
          const a = document.createElement('a');
          a.href = entry.pages_url;
          a.target = '_blank';
  ...
  async function loadArchiveSignals() {
    try {
      ...
      if (!resp.ok) throw new Error('signals/latest.json not found');
      const data = await resp.json();
      // If the file is a single object (not an array), wrap it so renderSignalCards can iterate
      if (data && !Array.isArray(data)) return [data];
      return data;
    } catch (e) {
      console.warn('Failed to load latest signals', e);
      return null;
    }
  }
