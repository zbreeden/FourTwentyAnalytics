...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function renderSignalCards(signals) {
    ...
    signals.forEach(sig => {
      ...
      title.textContent = sig.broadcast_name || sig.title || sig.name || sig.id || 'Signal';
      const meta = document.createElement('div');
      meta.className = 'signal-meta';
      meta.textContent = (sig.ts || sig.timestamp || sig.date || '') + (sig.moduleId ? ' • ' + sig.moduleId : '');
      const body = document.createElement('p');
      body.textContent = sig.broadcast_summary || sig.summary || sig.description || '';
