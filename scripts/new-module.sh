...
# Validate module key (snake_case)
if [ -z "$MODULE_KEY" ] || ! printf "%s" "$MODULE_KEY" | grep -Eq '^[a-z0-9_]+$'; then
  echo "❌ Invalid module key. Use lowercase letters, digits, and underscores only." >&2
  exit 1
fi

# Validate repo name (kebab-case)  
if [ -z "$REPO_NAME" ] || ! printf "%s" "$REPO_NAME" | grep -Eq '^[a-z0-9-]+$'; then
  echo "❌ Invalid repo name. Use lowercase letters, digits, and hyphens only." >&2
  exit 1
fi
...
if [ -f "seeds/modules.yml" ] && grep -q "key: $MODULE_KEY" "seeds/modules.yml"; then
  echo "❌ Module key '$MODULE_KEY' already exists in seeds/modules.yml" >&2
  exit 1
fi

# Create directory based on repo name
BASE="$REPO_NAME"
if [ -d "$BASE" ]; then
  echo "❌ Directory '$BASE' already exists" >&2
  exit 1
fi
...
if [ -f "seeds/emoji_palette.yml" ]; then
  ...
  while :; do
    ...
    # Check if glyph already exists in palette
    if grep -Fq -- "$EMOJI" seeds/emoji_palette.yml; then
      echo "⚠️ That glyph is already defined in seeds/emoji_palette.yml — please choose a different emoji or press Enter to skip."
      continue
    else
      break
    fi
